...
export const StudyPlans: React.FC = () => {
  ...
  const handleSubjectToggle = (subjCode: string) => {
    setSelectedSubjects((prev) => 
      prev.includes(subjCode) 
        ? prev.filter((s) => s !== subjCode) 
        : [...prev, subjCode]
    );
  };
  ...
  return (
    <div className="space-y-6 font-sans text-slate-800 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">
            Structure your schedule and stay on track with adaptive revision targets tailored for Cameroonian exams.
          </p>
        </div>
          ...
          {loading ? (
            ...
            <Card className="py-16 text-center space-y-4">
              ...
              <div className="space-y-1">
                <h3 className="font-extrabold text-slate-850 text-base">No revision tasks active</h3>
                <p className="text-xs text-slate-400 max-w-sm mx-auto">
                  Click the **Auto-Generate AI Plan** button above to structure your revision timeline instantly!
                </p>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <Card className="space-y-4">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  <h4 className="font-extrabold text-slate-800 text-sm uppercase tracking-wider">Today</h4>
              ...
              <Card className="space-y-4">
                <div className="flex items-center justify-between border-b border-slate-100 pb-3">
                  <h4 className="font-extrabold text-slate-800 text-sm uppercase tracking-wider">Tomorrow</h4>
        ...
        <div className="space-y-6">
          <Card className="p-6 text-left space-y-4">
            <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
              ...
              <h3 className="font-extrabold text-slate-800 text-sm uppercase tracking-wider">Active Study Plans</h3>
            </div>

            {plans.length === 0 ? (
              <p className="text-xs text-slate-400 py-6 text-center">No active study plans. Use the generator to create one.</p>
            ) : (
              <div className="space-y-3">
                {plans.map((plan) => (
                  <div key={plan.id} className="p-3.5 border border-slate-100 rounded-2xl bg-slate-50/30">
                    <h4 className="font-bold text-slate-800 text-sm mb-1">{plan.title}</h4>
                    <p className="text-[10px] text-slate-400 font-bold uppercase tracking-wider mb-2">Duration: {plan.startDate} to {plan.endDate}</p>
                    <div className="flex items-center justify-between text-xs border-t border-slate-100 pt-2 text-slate-500">
                      <span>{plan.tasks.length} Total Tasks</span>
      ...
      {/* 3. AI STUDY PLANNER GENERATOR OVERLAY MODAL */}
      {modalOpen && (
          ...
          <Card className="w-full max-w-lg p-8 bg-white rounded-3xl shadow-2xl relative border border-slate-150 animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex items-center gap-2.5 mb-6 text-left">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 font-medium">Select subjects to create a weekly adaptive revision plan</p>
              </div>
            </div>
            ...
            <form onSubmit={handleCreatePlan} className="space-y-6 text-left">
              ...
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-450 uppercase tracking-wider">Select Subjects</label>
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  <label className="font-bold text-slate-450 uppercase tracking-wider">Daily Study Hours</label>
              ...
              <div className="p-4 bg-slate-50 border border-slate-150 rounded-2xl flex gap-3 text-xs text-slate-500">
                ...
                <p className="leading-relaxed">
                  Ticha\'s AI engine will divide your chosen subjects into bite-sized daily revision milestones, automatically preparing quiz papers for review.
                </p>
              </div>
