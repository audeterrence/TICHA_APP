      ...
      {loading ? (
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
            {subjects.map((sub) => {
              const IconComp = iconMap[sub.icon] || BookOpen;
              ...
              return (
                ...
                >
                  ...
                  <div className="space-y-1">
                    <h4 className="text-xs font-bold text-slate-800 leading-tight truncate max-w-[100px]">{sub.name}</h4>
                    <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{sub.code}</span>
                  </div>
                  ...
                  <span className={`text-[10px] font-black mt-0.5 ${isActive ? 'text-tichaBlue' : 'text-slate-500'}`}>
                    {sub.mastery}% Done
                  </span>
                </div>
              );
  ...
  <span className="text-xs font-black text-slate-850">{activeSubject?.mastery}%</span>
              ...
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  ...
                  <h4 className="font-extrabold text-slate-800 text-sm uppercase tracking-wider">Syllabus Core Topics</h4>
                </div>
                ...
                {topicsLoading ? (
                  ...
                  <div className="text-center py-10">
                    <p className="text-xs text-slate-400 font-medium">No topics registered. Create subjects or take pop quizzes to populate stats!</p>
                  </div>
                ) : (
                    ...
                    {topics.map((top) => (
                      ...
                      >
                        ...
                        <div className="flex-1 space-y-2">
                          <div className="flex items-center gap-2">
                            <span className={`text-[10px] font-extrabold px-2.5 py-0.5 rounded-full ${getMasteryTextClass(top.mastery)}`}>
                              {top.mastery}% Mastered
                            </span>
                            <h5 className="font-bold text-slate-800 text-sm md:text-base leading-snug">{top.name}</h5>
                          </div>
                          ...
                          <div className="flex items-center gap-4">
                            <div className="flex-1 bg-slate-200/60 h-2 rounded-full overflow-hidden">
                              <div 
                                className={`h-full rounded-full transition-all duration-300 ${getMasteryColor(top.mastery)}`}
                                style={{ width: `${top.mastery}%` }} 
                              />
                            </div>
                            <span className="text-xs font-bold text-slate-450 shrink-0 w-8">{top.mastery}%</span>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between md:justify-end gap-6 shrink-0 border-t border-slate-100/50 md:border-0 pt-3 md:pt-0">
                          <div className="text-left md:text-right space-y-0.5">
                            <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wide">Accuracy Metrics</span>
                            <p className="text-xs font-bold text-slate-650">
                              <strong className="text-slate-800 font-extrabold">{top.correct}</strong> / {top.attempted} MCQs Correct
                            </p>
                          </div>
                          <Button 
